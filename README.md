...
It automatically generates configuration files with **space after `=`** for each receptor僕igand pair, logs output, and summarizes results in a CSV.
...
* Summary CSV with status for all receptor僕igand pairs.
...
* Assigns multiple receptor僕igand pairs to different threads.
...
* Runs multiple receptor僕igand pairs concurrently on **one GPU**.
